...
<td>
{% if k["active"] %}
<form action="/ban/{{ k['key'] }}" method="POST" style="display:inline">
<button class="orange">Ban</button>
</form>
{% else %}
<form action="/unban/{{ k['key'] }}" method="POST" style="display:inline">
<button class="blue">Unban</button>
</form>
{% endif %}

<form action="/delete/{{ k['key'] }}" method="POST" style="display:inline">
...
def generate():
    ...
    con.execute(
        "INSERT INTO keys (key, expiry, active, created) VALUES (?, ?, 1, ?)",
        (key, expiry, created)
    )
...
def add_key():
    ...
    con.execute(
        "INSERT OR REPLACE INTO keys (key, expiry, active, created) VALUES (?, ?, 1, ?)",
        (key, expiry, created)
    )
