...
def edit_key(key):
    ...
    if stopped: con.execute("UPDATE keys SET key=?,expiry=?,max_devices=?,paused_seconds=? WHERE key=?",(new_key,expiry,limit,new_seconds,old_key))
    else: con.execute("UPDATE keys SET key=?,expiry=?,max_devices=?,paused_seconds=NULL WHERE key=?",(new_key,expiry,limit,old_key))
