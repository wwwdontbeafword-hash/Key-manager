...
function toggleKeyboard(){document.getElementById("virtualKeyboard").classList.toggle("show");document.getElementById("keySearch").focus()}
function kbPress(v){let i=document.getElementById("keySearch");if(v==="⌫")i.value=i.value.slice(0,-1);else if(v==="CLEAR")i.value="";else i.value+=v;i.focus();filterKeys()}
function buildKeyboard(){let box=document.getElementById("virtualKeyboard");if(!box)return;["1234567890","QWERTYUIOP","ASDFGHJKL","ZXCVBNM"].forEach(chars=>{let r=document.createElement("div");r.className="kbRow";[...chars].forEach(c=>{let b=document.createElement("button");b.type="button";b.className="kbKey";b.textContent=c;b.onclick=()=>kbPress(c);r.appendChild(b)});box.appendChild(r)});let r=document.createElement("div");r.className="kbRow";["-","_","⌫","CLEAR"].forEach(c=>{let b=document.createElement("button");b.type="button";b.className="kbKey kbWide";b.textContent=c;b.onclick=()=>kbPress(c);r.appendChild(b)});box.appendChild(r)}
